...
Claude Desktop (2026-09-09) registered the plugin and started the
connector's OAuth handshake (see the Authentik table), but whether **CIWG
Knowledge** then appears under Connectors *in chat* has not been
confirmed. Until someone sees it there, use the custom-connector path
below for chat (and if the upload turns out to work, say so in the
release ticket so this section — and the note in `release.json` — can go).
...
slug `ciwg-knowledge`, serves everything: the custom connector added in
claude.ai / Desktop settings, this plugin's automatic and manual sign-in,
and Claude Code's built-in MCP OAuth (the client behind the Desktop zip's
connector, and behind `claude mcp add`).
...
| Redirect URIs | **strict** `https://claude.ai/api/mcp/auth_callback`, **strict** `https://claude.com/api/mcp/auth_callback`, **regex** `^http://127\.0\.0\.1(:\d+)?/callback$`, **regex** `^http://localhost:\d+/callback$` | The strict entries are the hosted Claude callbacks (the custom connector added by hand in claude.ai / Desktop settings). The `127.0.0.1` regex is what **this plugin's own sign-in** uses — the loopback listener binds `127.0.0.1` on a random port (RFC 8252). The `localhost` regex is what **Claude Code's built-in MCP OAuth** uses — and that is the client behind the Desktop zip's connector: after a plugin upload through Claude Desktop on 2026-09-09, the Connect handshake redirected to `http://localhost:<random port>/callback` and Authentik answered "Invalid redirect URI" until this entry existed. (Which Desktop surface then lists the connector's tools — Cowork, or chat too — is the separate, still-open check in "What runs where".) Regex mode, escaped dots, anchored |
| Scopes | `openid`, `profile`, `email`, **`offline_access`** | `offline_access` makes Authentik issue a refresh token; `profile`'s mapping carries `groups` for the staff gate. The plugin asks for `openid profile email groups offline_access` (`scripts/lib/auth.mjs`) — Authentik has no separate `groups` scope and tolerates the extra name (the live sign-in works with it) |
...
2024.8.5+ / 2024.10.3+ / 2024.12.x; re-check both regex entries' mode
after any upgrade.

Server side (ci-connect): the REST API and `/mcp` accept this
application's tokens (issuer + audience above); the web-services settings
are documented in ci-connect's `docs/auth/mcp-oauth-connector.md`.
